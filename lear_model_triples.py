...
dataset = "ERCOT_7"
years_test = 1
calibration_window = [7 * 12, 7 * 26, 364, 2 * 364, 4 * 364]
## Calibration window Set to 2 weeks - it was set to 4 * 364
## Calibration window for 1 month test - 7 * 24 * 2 (had to be two weeks)
...
# -------------------------- PARARELLIZE THE TASK HERE ----------------------------------#
for idx in range(task_id + 1, num_tasks + 1, len(calibration_window)):
    window = calibration_window[idx]
    ...
    forecast_file_name = str(dataset) + \
                         '_LEAR_' + str(window) + '_days.csv'
    ...
    ### Now actually define a model and run it
    model = LEAR(calibration_window=window)
    ...
    ## Loop over the dates to be forecasted
    for date in forecast_dates:
        ...
        # error_df = error_df.append({'Date': date, 'sMAPE': smape, 'MAE': mae}, ignore_index=True)
    ...
    # Print out the times
    print(f"The runtime of a LEAR model with a window of {window} days is: {runtime}")

    ## Filename
    file_name = f"LEAR_Forecast_run_{window}_window_days.csv"

    # Save the error dataframe
    error_df.to_csv(os.path.join(path_recalibration_folder, file_name))
