...
hyperparameter_evals = [300, 500, 1000, 1500]
...
#---------------------------------- START PARARELLIZING -------------------------------#
for idx in range(task_id + 1, num_tasks + 1, len(hyperparameter_evals)):
    hyp_evals = hyperparameter_evals[idx]
    ...
    _dnn_hyperopt.hyperparameter_optimizer(path_datasets_folder=path_datasets_folder,
                                           ...
                                           new_hyperopt=1,max_evals=hyp_evals,
    ...
    print(f"The runtime of a DNN model with hyp_evals of {hyp_evals} evaluations is: {runtime}")
    
    # Save the error dataframe
    error_df.to_csv(os.path.join(path_recalibration_folder, f"DNN_Error_run_{hyp_evals}_evals_{nlayers}_{current_datetime.strftime('%Y-%m-%d_%H-%M-%S')}.csv"), index=False)
