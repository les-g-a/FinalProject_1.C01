...
from keras.layers import Bidirectional, LSTM, Dropout, Dense, TimeDistributed, Lambda
...
def preprocess(chop, lookback, forecast, scaler, n_splits=3, val_size=0.2, test_size=0.1, name = None):
    ...
    df_dam_pred = pd.read_csv('final_input_with_dampred.csv')
...
def run_model(X_train_encoded, y_train_encoded, X_val_encoded, y_val_encoded, epochs, run_name, model_name = 'BiLSTM_3layers', regularizer=0.0001, dropout=0.2):
    ...
    input_shape = (batch_size,X_train_encoded.shape[1], X_train_encoded.shape[2])  # (num_timesteps per sequence, num_features)
...
def predict(X_test_encoded, y_test_encoded, BiLSTM_3layers, Yscaler, run_name=''):
    ...
    pred_path = 'Model_Outputs/predictions_BiLSTM_3layers_' + dt.datetime.now().strftime('%Y-%m-%d_%H-%M-%S_') + run_name + '.csv'
...
def predict_continuous_24hr(X_test_encoded, y_test_encoded, BiLSTM_3layers, Yscaler, run_name=''):
    # Initialize a list to hold all concatenated predictions
    all_predictions = []
    all_actuals = []

    print("Num of seq: ", X_test_encoded[0])
    # Iterate through each test sequence
    for i in range(len(X_test_encoded[0])):
        # Make prediction for the current sequence
        single_prediction = BiLSTM_3layers.predict(X_test_encoded[i:i+1])
        # Extract the first 24 timesteps from the prediction
        first_24_hours = single_prediction[:, :24, :].reshape(-1, 1)
        # Inverse scale the predictions
        scaled_predictions = Yscaler.inverse_transform(first_24_hours)
        # Store scaled predictions
        all_predictions.extend(scaled_predictions.flatten())

        # Prepare actuals for comparison, extract corresponding 24 hours
        actual_24_hours = y_test_encoded[i, :24, :].reshape(-1, 1)
        actual_scaled = Yscaler.inverse_transform(actual_24_hours)
        all_actuals.extend(actual_scaled.flatten())

    # Ensure predictions and actuals are the same length
    min_length = min(len(all_predictions), len(all_actuals))
    all_predictions = np.array(all_predictions)[:min_length]
    all_actuals = np.array(all_actuals)[:min_length]

    # Create DataFrame to compare actual vs predicted values
    pred_df = pd.DataFrame({'Actual': all_actuals, 'Predicted': all_predictions})
    pred_path = 'Model_Outputs/predictions_continuous_24hr_BiLSTM_3layers_' + dt.datetime.now().strftime('%Y-%m-%d_%H-%M-%S_') + run_name + '.csv'
    pred_df.to_csv(pred_path)
    print("Saved Prediction at: ", pred_path)
